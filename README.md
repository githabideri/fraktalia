# Fraktalia - Group Chat Setup

## Overview
Fraktalia is a Matrix room for friends, powered by Mox agent.

## Room Details
- **Room ID:** `!ivMECwfpEuKXhIWAny:matrixbot.home.macl.at`
- **Agent:** Mox (id: `mox`)
- **Model:** Opus (fallback: Sonnet)
- **Sandbox:** Enabled (full isolation)

## Access
- Friends connect via **Tailscale Magic DNS**: `https://clawdbot.tail51128.ts.net:8448`
- The local domain `matrixbot.home.macl.at` is **not accessible** to friends (pihole/split DNS only)
- Registration requires single-use tokens
- Users end up as `@username:matrixbot.home.macl.at` regardless of access URL
...
- Registration token (single-use, get from m)
...
2. **Register an account**
   - Homeserver URL: `https://clawdbot.tail51128.ts.net:8448`
   - Click "Create account" or "Register"
   - Enter username, password, and registration token
   - You'll become `@username:matrixbot.home.macl.at`

3. **Join Fraktalia**
   - Wait for an invite from m, or
   - Ask m for a direct invite to the room

4. **Chat with Mox**
   - Just talk in the room - Mox responds when it has something to say
   - No @mention needed
...
| "Invalid token" on registration | Token already used or typo | Get new token from m |
| Can't find room | Not invited yet | Ask m for invite |
| Mox doesn't respond | Nothing worth saying, or bot offline | That's fine - Mox speaks when it counts |
...
```
~/.clawdbot/matrix-admin-token
```

### Check existing tokens
```bash
TOKEN=$(cat ~/.clawdbot/matrix-admin-token)
...
```bash
# Password stored securely at ~/.clawdbot/matrix-admin-password
PW=$(cat ~/.clawdbot/matrix-admin-password)
NEW_TOKEN=$(curl -s -X POST "http://localhost:8008/_matrix/client/r0/login" \
  -H "Content-Type: application/json" \
  -d "{\"type\": \"m.login.password\", \"user\": \"matrixadmin\", \"password\": \"$PW\"}" \
  | jq -r '.access_token')
echo "$NEW_TOKEN" > ~/.clawdbot/matrix-admin-token
chmod 600 ~/.clawdbot/matrix-admin-token
```
...
**What a compromised/misbehaving Mox CAN do:**
...
| Capability | Status |
|------------|--------|
| Docker image | `mox-sandbox:latest` |
...
Build location: `docker/mox-sandbox/`

```bash
# Rebuild if needed:
cd /var/lib/clawdbot/workspace/docker/mox-sandbox
docker build -t mox-sandbox:latest .
```
...
Mox has **two separate workspace directories**:

| Location | Purpose | Edited by |
|----------|---------|-----------|
| `/var/lib/clawdbot/workspace/agents/mox/` | Main workspace (admin config) | You |
| `/var/lib/clawdbot/.clawdbot/sandboxes/agent-mox-{hash}/` | Sandbox workspace (runtime) | Mox |
...
| `memory/` | Sandbox only | Never synced (Mox's notes) |
| Code/files Mox creates | Sandbox only | Persists, never synced |

### Design Decision: No Self-Improvement

We chose **admin-controlled instructions** over self-improvement:

**Option A (chosen):** Admin edits config → synced to sandbox → overwrites any Mox changes
- ✅ Predictable behavior
- ✅ Admin always in control
- ✅ Simple mental model
- ❌ Mox can't evolve its own instructions

**Option B (rejected):** Bidirectional sync / self-improvement
- ✅ Mox could refine its own personality/instructions
...
1. Edit files in **main workspace**: `/var/lib/clawdbot/workspace/agents/mox/`
...
When you need to completely reset Fraktalia (new persona, fresh start, etc.):

```bash
TOKEN=$(cat /var/lib/clawdbot/.clawdbot/matrix-admin-token)
ROOM="!ivMECwfpEuKXhIWAny:matrixbot.home.macl.at"
...
for mid in $(curl -s "http://localhost:8008/_synapse/admin/v1/room/$ROOM/media" \
  ...
  curl -X DELETE "http://localhost:8008/_synapse/admin/v1/media/matrixbot.home.macl.at/$mid" \
    -H "Authorization: Bearer $TOKEN"
done
...
# 3. Reset Mox session (see below)
```

### Session Reset Only (Keep Matrix History)

For just resetting Mox's context without touching Matrix:

```python
# /var/lib/clawdbot/.clawdbot/agents/mox/sessions/sessions.json
# Set systemSent: false and generate new sessionId
# Or run from Felix: "reset Mox session"
```
...
| Firewall rules | On reboot | mox-firewall.service (systemd) |
...
```json
"groupAllowFrom": ["@m:matrixbot.home.macl.at", "@friend1:matrixbot.home.macl.at"]
```
...
- **Script:** `/var/lib/clawdbot/workspace/scripts/matrix-room-monitor.sh`
...
Mox has internet access but is blocked from LAN/Tailscale:

**Docker network:** `mox-internet`
**Config:** `sandbox.docker.network: "mox-internet"`

### Firewall Setup (requires root)

```bash
# One-time setup - run as root on the LXC host:
sudo /var/lib/clawdbot/workspace/scripts/mox-network-firewall.sh
```
...
Already configured: `mox-firewall.service` runs automatically after Docker on boot.

```bash
# Check status
systemctl status mox-firewall.service

# Manual re-run if needed
sudo systemctl restart mox-firewall.service
```

Service file: `/etc/systemd/system/mox-firewall.service`
...
- Invite yourself (@m:...)
- Invite a helper account (@roomhelper:... or @synadmin:...)
- Bot is already there

### 3. Create Agent Workspace
```bash
mkdir -p /var/lib/clawdbot/workspace/agents/<agent-id>
# Create: AGENTS.md, SOUL.md, IDENTITY.md, USER.md, TOOLS.md
```

### 4. Build Docker Image (if sandboxed)
```bash
cd /var/lib/clawdbot/workspace/docker/<agent>-sandbox
...
```json
{
  "agents": {
    "list": [
      {
        "id": "<agent-id>",
        "workspace": "/var/lib/clawdbot/workspace/agents/<agent-id>",
...
```bash
BOT_TOKEN="<token>"
curl -X PUT "http://localhost:8008/_matrix/client/v3/rooms/!roomid:server/state/m.room.member/@clawdbot:server" \
  -H "Authorization: Bearer $BOT_TOKEN" \
  -H "Content-Type: application/json" \
...
3. Configured Mox agent with Opus + sandbox
4. Set up room monitor cron
5. Created registration tokens for friends
6. Added internet-only network access (mox-internet)
7. Created secure admin user (`matrixadmin`) with token stored at `~/.clawdbot/matrix-admin-token`
